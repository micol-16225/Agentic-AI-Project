...
class BiostatLifecycleAgent3:
    def __init__(self, api_key, library_path, model_id="llama-3.3-70b-versatile"):
...
class BiostatLifecycleAgent3:
    ...
    def audit_protocol(self, user_protocol, historical_lessons, user_directives=""):
        ...
        # --- 1. CLEAN THE DATA (The "nan" Fix) ---
        # If title is missing, combine Source + Date, or use a snippet of content
        df['title'] = df['title'].fillna(
            df['source'].astype(str) + "_" + df['date'].astype(str)
        ).replace("nan_nan", "FDA_Historical_Letter")

        # --- 2. LOAD STATUTORY (The Permanent Resident) ---
        stat_mask = (df['type'] == 'Statutory') | (df['type'] == 'Academic_Rigor')
        fixed_docs = df[stat_mask]
        fixed_context = "### MANDATORY STATUTORY & ACADEMIC RULES ###\n" + "\n\n".join([
            f"ID: {row['title']}\n{row['content']}" for _, row in fixed_docs.iterrows()
        ])

        # --- 3. LOCAL RESEARCHER (Picking the best letter) ---
        ...
        try:
            ...
            # Pulling the TOP 3 most relevant full letters
            top_indices = sims[0].argsort()[-2:][::-1]
            prec_list = []
            for idx in top_indices:
                row = letter_df.iloc[idx]
                prec_list.append(f"SOURCE ID: {row['title']}\n{row['content']}")
            
            prec_context = "### TOP 3 FDA HISTORICAL PRECEDENTS ###\n\n" + "\n\n---\n\n".join(prec_list)
            print(f"✅ Local Researcher selected 3 letters: {', '.join(letter_df.iloc[top_indices]['title'].tolist())}")
        except:
            prec_context = "Historical precedents unavailable."

        self.knowledge_base = f"{fixed_context}\n\n{prec_context}"
        ...
        YOUR OBJECTIVE: Identify any statistical or regulatory risks that would lead to an FDA Refusal to File (RTF).
            CATEGORIES TO ASSESS, AMONG OTHERS:   
        ...
        ### TRUTH ANCHORING RULES (MANDATORY):
        1. You MUST use "double quotes" ONLY for word-for-word matches from the INTEGRATED KNOWLEDGE BASE above.
        2. If you cite the PROTOCOL, use [[double brackets]].
        3. Do not use your internal memory to finish a quote. If it's not in the text above, don't quote it.
        4. VALIDATION WARNING: Any text in "double quotes" that is not found in the Library will be marked as a Hallucination, and considered not valid for a diligent auditor.
        5. Is there a specific match in the INTEGRATED KNOWLEDGE BASE? 
           - IF YES: Quote it using "DOUBLE QUOTES" and cite the Source ID.
           - IF NO: State explicitly "NO MATCH FOUND IN LIBRARY. Using general regulatory principles.". Don't put anything in double quotes.
        
        CRITICAL: Never put Protocol text in "double quotes". Never attribute Protocol sentences to an FDA Source ID.

        STRUCTURE YOUR AUDIT REPORT INTO FOUR SECTIONS FOLLOWING THE INSTRUCTIONS AND RULES ABOVE:
        ---
        ...
        A. The Law and Regulations (ICH E9, 21 CFR, HITL Gates).
        B. The Academic Literature (e.g., Akacha, Mallinckrodt).
        
        If a method in the protocol violates a rule or academic standard, you MUST:

        1. Cite the exact Title of the rule/paper from the Knowledge Base.
        2. Provide a verbatim quote from the 'content' field of that entry. Write in bold characters only the relevant technical requirement (e.g., 'Type I Error control') that proves the violation.
        3. Map this quote directly to the sentence in the protocol you cited in your Internal Monologue.
    ...
    def _generate_response(self, prompt, max_retries=5):
        ...
        while retries < max_retries:
            try:
                # FIX: Use self.model_id instead of a hardcoded string
                ...
                # Maintain your LaTeX cleaning logic
                clean_response = content.replace("\\(", "$").replace("\\)", "$")
                clean_response = clean_response.replace("\\[", "$$").replace("\\]", "$$")
                return clean_response
                ...
                if "429" in str(e):
                    ...
                    print(f"⚠️ Rate Limit Hit (429). Retrying in {wait_time:.2f}s...")
                    time.sleep(wait_time)
                    retries += 1
                # Handle the Deprecation/400 error specifically if needed, 
                # but using self.model_id solves the root cause.
                else:
                    raise e 
                    
        raise Exception("Max retries exceeded. API is heavily throttled.")
